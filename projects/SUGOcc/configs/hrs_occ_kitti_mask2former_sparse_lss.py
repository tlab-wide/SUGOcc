...
model = dict(
    ...
    img_view_transformer=dict(
        ...
        vp_megvii=False,
        trilinear_interp=False,
        dsf_embedding=True,
        occ_supervision=False,
    ...
    img_bev_encoder_neck=dict(
        # type='CascadeDensifyPixelDecoder',
...
train_pipeline = [
    ...
    dict(type='CustomPack3DDetInputs', keys=['img_inputs', 'gt_occ'], 
         meta_keys=['pc_range', 'occ_size', 'lidar2img', 'cam_intrinsic']),
]
...
test_pipeline = [
    ...
    dict(type='OccDefaultFormatBundle3D'), 
    dict(type='CustomPack3DDetInputs', keys=['img_inputs', 'gt_occ'], 
         meta_keys=['pc_range', 'occ_size', 'lidar2img', 
                    'cam_intrinsic', 'sequence', 'frame_id', 'raw_img']),
]
...
default_hooks = dict(
    ...
    # profiler=dict(type='ProfilerHook', by_epoch=False, 
    #               with_flops=True, on_trace_ready=trace_config),
)
# profiler_hook_cfg = dict(by_epoch=False)
# custom_hooks = [
#     dict(
#         type='MEGVIIEMAHook',
#         init_updates=10560,
