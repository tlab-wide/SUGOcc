...
train_pipeline = [
    ...
    dict(
        ...
        meta_keys=['pc_range', 'occ_size','sample_idx','filename','flag']),
]
...
train_dataloader = dict(
    ...
    batch_sampler=dict(type='GroupInBatchSampler', batch_size=samples_per_gpu),
    dataset=dict(
        type=dataset_type,
        data_root=data_root,
...
# learning policy
param_scheduler = [
    dict(type='MultiStepLR', by_epoch=False, milestones=[num_epochs * num_iters_per_epoch], gamma=0.1),
...
train_cfg = dict(by_epoch=False, max_iters=num_epochs * num_iters_per_epoch, val_interval=num_iters_per_epoch)
val_cfg = dict()
test_cfg = dict()
