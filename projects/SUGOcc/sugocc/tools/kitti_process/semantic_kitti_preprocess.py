...
def process_label(label, voxel_size=(240, 144, 240), downscale=4, label_filename="123"):
    LABEL_ds = _downsample_label(label, (256, 256, 32), downscale)
            ...
            for scale in downscaling:
                ...
                if True:
                    if scale != "1_1":
                        ...
                        # )
                        # print("wrote to", label_filename)
                        p.apply_async(process_label, (LABEL, (256, 256, 32), downscaling[scale], label_filename))
                    else:
                        LABEL_ds = LABEL
                        np.save(label_filename, LABEL_ds)
                        print("wrote to", label_filename)
