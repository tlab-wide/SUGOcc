...
class CustomFPN(BaseModule):
    ...
    def forward(self, inputs):
        ...
        if self.with_cp and inputs[0].requires_grad:
            laterals = [
                checkpoint(lateral_conv, inputs[i + self.start_level])
        ...
        # import pdb;pdb.set_trace()
        if self.with_cp and laterals[0].requires_grad:
            outs = [checkpoint(self.fpn_convs[i],laterals[i]) for i in self.out_ids]
        else:
...
class SimpleUnet(nn.Module):
    def __init__(self,
        ...
        self.down1 = nn.Sequential(
            nn.Conv3d(in_channels, in_channels*2, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
        )
        self.down2 = nn.Sequential(
            nn.Conv3d(in_channels*2, in_channels*4, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
        )
        self.down3 = nn.Sequential(
            nn.Conv3d(in_channels*4, in_channels*8, kernel_size=3, stride=2, padding=1),
            nn.ReLU(inplace=True),
        )
    ...
    def forward(self, feats):
        ...
        x_64 = feats
        if self.with_cp:
            x_32 = checkpoint(self.down1, x_64)
            x_16 = checkpoint(self.down2, x_32)
            x_8 = checkpoint(self.down3, x_16)
        else:
            x_32 = self.down1(x_64)
            x_16 = self.down2(x_32)
            x_8 = self.down3(x_16)
        x_32= self.up1(x_32)
        x_16= self.up2(x_16)
        x_8= self.up3(x_8)
