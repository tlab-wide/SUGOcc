...
class ViewTransformerLiftSplatShoot(BaseModule):
    ...
    def get_geometry(self, rots, trans, intrins, post_rots, post_trans, bda):
        ...
        points = torch.inverse(post_rots.float()).view(B, N, 1, 1, 1, 3, 3).matmul(points.unsqueeze(-1))
        ...
        combine = rots.matmul(torch.inverse(intrins.float()))
...
class ViewTransformerLSSBEVDepth(ViewTransformerLiftSplatShoot):
    ...
    def get_mlp_input(self, rot, tran, intrin, post_rot, post_tran, bda=None):
        ...
        if rot.shape[-1] == 4:
            ...
            sensor2ego = torch.cat([rot[:, :, :3, :3], rot[:, :, :3, 3].reshape(B, N, 3, 1)], dim=-1).reshape(B, N, -1)
        else:
            # for KITTI, the rot matrix is 3x4
