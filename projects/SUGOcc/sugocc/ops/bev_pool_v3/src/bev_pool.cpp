...
void bev_pool_v3(int c, int n_intervals, const float* depth, const float* feat,
    const float* depth_embed, const int* ranks_depth, const int* ranks_feat,
...
void bev_pool_v3_grad(int c, int n_intervals, const float* out_grad,
  ...
  const int* ranks_bev, const int* interval_starts, const int* interval_lengths,
  float* depth_grad, float* feat_grad, float* depth_embed_grad);
...
void bev_pool_v3_forward(
  ...
  const at::Tensor _depth_embed,   // (B, N, D, fH, fW, C)
  ...
  const float* depth_embed = _depth_embed.data_ptr<float>();
...
void bev_pool_v3_backward(
  ...
  at::Tensor _depth_embed_grad,
  const at::Tensor _depth,
  const at::Tensor _feat,
  const at::Tensor _depth_embed,
  ...
  const int* interval_lengths = _interval_lengths.data_ptr<int>();
  const int* interval_starts = _interval_starts.data_ptr<int>();
  float* depth_embed_grad = _depth_embed_grad.data_ptr<float>();
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("bev_pool_v3_forward", &bev_pool_v3_forward,
        "bev_pool_v3_forward");
  m.def("bev_pool_v3_backward", &bev_pool_v3_backward,
        "bev_pool_v3_backward");
}
