...
*/
__global__ void bev_pool_v3_kernel(int c, int n_intervals,
  ...
  for(int i = 0; i < interval_length; i++){
    ...
    //   continue;
    // }
    cur_depth_embed = depth_embed + ranks_depth[interval_start+i] * c + cur_c;
    // ranks_feat[interval_start+i]: feature索引, 介于(0, B*N*fH*fW-1)之间.
    cur_feat = feat + ranks_feat[interval_start+i] * c + cur_c;  //
    psum += *cur_feat * *cur_depth + *cur_depth_embed;  // 聚合该pillar对应的cur_c特征.
  }
...
__global__ void bev_pool_grad_kernel(int c, int n_intervals,
                                  ...
                                  float* __restrict__ depth_embed_grad) {
...
void bev_pool_v3(int c, int n_intervals, const float* depth, const float* feat, const float* depth_embed, const int* ranks_depth,
  const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* out) {
  bev_pool_v3_kernel<<<(int)ceil(((double)n_intervals * c / 256)), 256>>>(
    c, n_intervals, depth, feat, depth_embed, ranks_depth, ranks_feat,
    ranks_bev, interval_starts, interval_lengths, out
  );
}

void bev_pool_v3_grad(int c, int n_intervals, const float* out_grad,
  const float* depth, const float* feat, const float* depth_embed, const int* ranks_depth, const int* ranks_feat,
  const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* depth_grad, float* feat_grad, float* depth_embed_grad) {
  bev_pool_grad_kernel<<<(int)ceil(((double)n_intervals / 256)), 256>>>(
     c, n_intervals, out_grad, depth, feat, depth_embed, ranks_depth, ranks_feat,
     ranks_bev, interval_starts, interval_lengths, depth_grad, feat_grad, depth_embed_grad
  );
}
