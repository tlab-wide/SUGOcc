...
from projects.SUGOcc.sugocc.ops.occ_pooling import occ_pool_ext
...
class QuickCumAvgCuda(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x, geom_feats, ranks, B, D, H, W):
        kept = torch.ones(x.shape[0], device=x.device, dtype=torch.bool)
        kept[1:] = ranks[1:] != ranks[:-1]
        ...
        interval_lengths[-1] = x.shape[0] - interval_starts[-1]
        geom_feats = geom_feats.int()
        out = occ_pool_ext.occ_avg_pool_forward(
            x,
            geom_feats,
            interval_lengths,
            interval_starts,
    ...
    def backward(ctx, out_grad):
        ...
        out_grad = out_grad.contiguous()
        x_grad = occ_pool_ext.occ_avg_pool_backward(
            out_grad,
            geom_feats,
            interval_lengths,
            interval_starts,
...
def occ_avg_pool(feats, coords, B, D, H, W):
    assert feats.shape[0] == coords.shape[0]
    ...
    indices = ranks.argsort()
    feats, coords, ranks = feats[indices], coords[indices], ranks[indices]
    # print(feats.shape, coords.shape, ranks.shape)
    x = QuickCumAvgCuda.apply(feats, coords, ranks, B, D, H, W)
    x = x.permute(0, 4, 1, 2, 3).contiguous()
    return x