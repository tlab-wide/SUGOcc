...
at::Tensor occ_avg_pool_forward(
  const at::Tensor _x,
  const at::Tensor _geom_feats, 
  const at::Tensor _interval_lengths, 
  const at::Tensor _interval_starts,
...
/*
  ...
  Return:
    x_grad           : output features, FloatTensor[n, 4]
*/
at::Tensor occ_avg_pool_backward(
  const at::Tensor _out_grad,
  const at::Tensor _geom_feats, 
  const at::Tensor _interval_lengths, 
  const at::Tensor _interval_starts,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def("occ_pool_backward", &occ_pool_backward,
        "occ_pool_backward");
  m.def("occ_avg_pool_forward", &occ_avg_pool_forward,
        "occ_avg_pool_forward");
  m.def("occ_avg_pool_backward", &occ_avg_pool_backward,
        "occ_avg_pool_backward");
}