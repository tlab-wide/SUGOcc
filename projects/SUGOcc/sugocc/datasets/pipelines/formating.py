    ...
    def __call__(self, results):
        ...
        if 'gt_occ' in results.keys() and results['gt_occ'] is not None:
            if type(results['gt_occ']) is list:
                results['gt_occ'] = tuple([to_tensor(x) for x in results['gt_occ']])
            else:
                results['gt_occ'] = to_tensor(results['gt_occ'])

        if 'points_occ' in results.keys():
            results['points_occ'] = to_tensor(results['points_occ'])
...
class CustomPack3DDetInputs(BaseTransform):
    INPUTS_KEYS = ['points', 'img', 'img_inputs', 'gt_occ']
