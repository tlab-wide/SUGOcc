...
class CustomSemanticKITTILssDataset(BaseDataset):
    ...
    def __init__(self, split, camera_used, occ_size, pc_range, data_root, ann_file, classes,
                 pipeline=[], test_mode=False, load_continuous=False, *args, **kwargs):
    ...
    def _set_sequence_group_flag(self):
        """Set flag according to sequence id.

        Sequences with id greater than 10 will be set as group 1,
        otherwise group 0.
        """
        res = []
        curr_sequence = 0
        ...
        for idx in range(len(self)):
            ...
            sequence_id = int(info['sequence'])
            if sequence_id != curr_sequence:
    ...
    def get_augmentation(self):
        ...
        if np.random.uniform() < 0.5:
            aug_config['flip_x'] = True
        if np.random.uniform() < 0.5:
            aug_config['flip_y'] = True
        aug_config['rot_angle'] = np.random.uniform(-np.pi / 4, np.pi / 4)
        aug_config['scale_factor'] = np.random.uniform(0.95, 1.05)
        return None
    ...
    def load_data_list(self):
        ...
        for sequence in self.sequences:
            ...
            for id_path in glob.glob(id_base_path):
                ...
                for i in self.multi_scales:
                    voxel_paths.append(os.path.join(voxel_base_path, img_id + f'_{i}.npy'))
    ...
    def get_ann_info(self, index):
        info = self.data_list[index]['voxel_path']
        gts = []
        for path in self.data_list[index]['voxel_paths']:
            if path is not None:
                gts.append(np.load(path))
        return gts
        ...
        if 'ssc_scores' in results:
            ...
            if 'ssc_results_refine' in results:
                ...
                res_dic_refine = {
                    ...
                    "SC_refine_IoU": iou,
                    "SSC_refine_mIoU": iou_ssc[1:].mean(),
                }
